...
from shapenet.config import get_kostas_cluster_shapenet_cfg
...
class ClusterStateManager:
    def __init__(self, logger, time_to_run=600):
    ...
    def timer_handler(self, signal, frame):
        self.logger.info(f"Received alarm [{signal}]")
    ...
    def get_exit_code(self):
        ...
        if self.external_exit is not None:
            ...
            # If the job preempts it means that other job took over the resources
            # but the training has not completed yed. Therefore it should be
            # re-scheduled to run.
...
def main_worker(worker_id, args):
    ...
    # start the Cluser State Manager
    csm = ClusterStateManager( logger=logger, time_to_run=cfg.CLUSTER.TIME_TO_RUN)
    ...
    training_loop(cfg, csm, cp, model, optimizer, scheduler, loaders, device, loss_fn)

    # if the code reaches this points means that the training is over and should appropriately exit the script
    return 0


def training_loop(cfg, csm, cp, model, optimizer, scheduler, loaders, device, loss_fn):
    Timer.timing = False
...
def setup(args):
    ...
    """
    cfg = get_kostas_cluster_shapenet_cfg()
